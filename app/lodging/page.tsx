...
export default async function LodgingPage() {
  ...
  const actual = stays.reduce((s, l) => s + (l.actualCents ?? l.plannedCents), 0);
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 md:px-6 md:py-14">
      <Reveal>
        ...
        <p className="mt-3 max-w-xl text-sm text-ink-secondary md:text-base">
          Every Best Western is on John&apos;s employee rate. Budgeted{" "}
          <span className="text-ink line-through decoration-ink-muted/60">{fmtMoney(planned)}</span>{" "}
          — tracking at <span className="font-medium text-mark-green">{fmtMoney(actual)}</span>.
        </p>
      </Reveal>
